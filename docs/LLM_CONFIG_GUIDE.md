...
- 本地 CLI 每次调用使用独立临时 cwd。Windows 上 CLI 子进程退出后仍可能短暂占用该目录，导致 cleanup `PermissionError`；此时若生成已成功，DSA 会忽略清理错误并继续返回结果（diagnostics 记为 `temp_cleanup_error`），不会误报 `unknown_backend_error`。
