...
- Each local CLI call uses an isolated temporary cwd. On Windows, CLI child processes may still hold that directory briefly after exit, causing cleanup `PermissionError`. When generation already succeeded, DSA ignores the cleanup error and still returns the result (recorded as `temp_cleanup_error` in diagnostics) instead of misreporting `unknown_backend_error`.
